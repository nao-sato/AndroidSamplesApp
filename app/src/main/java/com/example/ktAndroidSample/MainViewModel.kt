...
class MainViewModel: ViewModel() {
    val activityTriple = MutableLiveData<List<Triple<String, String, Int>>>()
    val clickGuid = MutableLiveData<Int>()

    fun initData() {
        activityTriple.postValue(
                listOf(
                    ...
                    Triple("ImplicitIntent",MapActivity::class.java.simpleName,R.string.ImplicitIntent_desc)
                )
        )
    }
    ...
    fun initDialog(data:Int,context: Context){
        MaterialDialog(context).show {
            when(data){
                ...
                R.string.ImplicitIntent_desc -> title(R.string.ImplicitIntent_name)
            }
            message(data)
